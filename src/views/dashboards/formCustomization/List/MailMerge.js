...
// === Table Data ===
// Includes section rows (type: 'section') and data rows (type: 'row')
// Values use 'Y'/'N' strings for toggle compatibility
const initialRows = [
  { id: 1, type: 'section', field: 'Create New Templete' },
  {
    id: 2,
    type: 'row',
    field: 'Templete Name',
    roles: { User: 'N', AE: 'Y', PM: 'N', ME: 'N', QE: 'N', BDM: 'Y', Admin: 'N', Supplier: 'N' }
  },
  {
    id: 3,
    type: 'row',
  ...
  { id: 5, type: 'section', field: 'Add Signature' },
  {
    id: 6,
    type: 'row',
        ...
        {rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(row => {
          if (row.type === 'section') {
            return (
              ...
              >
                <Typography sx={{ fontWeight: 700 }}>{row.field}</Typography>
              </Paper>
            )
          }
                    ...
                    return (
                      <Grid key={`${row.id}-${roleKey}`} item xs={6}>
                        ...
                        >
                          ...
                          <GreenCheckbox
                            checked={checked}
                            onChange={() => handleToggle(row.id, roleKey)}
                            size='small'
                          />
                        </Paper>
                      </Grid>
          ...
          <TableBody>
            {visibleRows.map(row => {
              if (row.type === 'section') {
                ...
                return (
                  <TableRow key={row.id} sx={{ backgroundColor: '#f6f9ff' }}>
                    <TableCell
              ...
              return (
                <TableRow key={row.id} hover sx={{ height: BODY_ROW_HEIGHT }}>
                  ...
                  >
                    <div
                      className='cellInner'
                      style={{ justifyContent: 'flex-start', paddingLeft: 16, flexWrap: 'wrap' }}
                    >
                      {row.field}
                    </div>
                  </TableCell>
